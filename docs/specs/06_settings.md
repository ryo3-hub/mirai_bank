...
- 1 つも選んでいないと「曜日を 1 つ以上選んでください（現在は通知されません）」エラー文を表示

#### 達成通知
- このスイッチが制御するのは **プッシュ通知のみ**（issue #197）
- スイッチ ON: 目標達成・節目時にプッシュ通知 + アプリ内ダイアログ演出
- スイッチ OFF: プッシュ通知なし。アプリ内ダイアログ演出は常に表示される
  （OFF 時に演出までスキップすると達成マークだけ付いて演出が永久に失われるため）
