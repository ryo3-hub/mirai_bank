...
## 概要
初回インストール時のみ表示される 4 ステップの全画面フォーム。

1. **イントロステップ**（情報提示のみ）: issue #125 で追加。アプリの目的と
   コアバリューを伝えてから設定フローに進ませる。「はじめる」をタップで次へ
2. **カテゴリ設定ステップ**（必須）: 最初のカテゴリと時給を登録、または「あとで設定する」でスキップ
3. **目標設定ステップ**（任意）: issue #102 で追加。カテゴリを実際に作った場合のみ続けて表示。プリセット（短期/中期/長期）から目標を選ぶ。スキップ可能
4. **リマインダー曜日選択ステップ**（任意）: issue #121 で追加。目標を実際に設定した場合のみ続けて表示。学習する曜日を選んでリマインダーを有効化、またはスキップ

カテゴリをスキップしたユーザーには目標ステップ・リマインダーステップは出さない（カテゴリ無しでは時給ベースの目標金額が計算できないため）。目標もスキップした場合はリマインダーステップも出さない（途中でスキップ＝もう何も設定しないユーザー意図とみなすため）。イントロステップにスキップ動線は無い（タップして必ずカテゴリ設定へ進む）。
...
## ステップ 0: イントロ（issue #125）

初回起動時に必ず一番先に表示される。アプリ説明だけのページで、設定操作は無い。

### UI 構成
```
[Scaffold]
  body: SingleChildScrollView
    - 大きめ円形アイコン（96px、Icons.savings_outlined、primary 12% 背景）
    - 見出し「自己投資の価値を、見える化しよう」（headlineSmall、w800、2 行）
    - 概要本文（bodyMedium、onSurfaceVariant、中央寄せ）
    - コアバリュー 3 項目（_IntroPoint × 3）
      - カテゴリごとに時給を決める
      - タイマーで集中時間を記録
      - 積み上げを「資産」として実感
    - SaveActionButton「はじめる」（Icons.arrow_forward）
```

### アクション
- 「はじめる」タップ → `_step = _OnboardingStep.category` でカテゴリ設定へ
- スキップ動線は無い（「あとで」相当はカテゴリ設定ステップにある）
- カテゴリ設定からイントロに戻る導線も無い（一方通行）

### 文言
要件定義（CLAUDE.md §1）に基づく：
- 見出し: 「自己投資の価値を、見える化しよう」
- 本文: 「勉強やスキルアップは、未来の自分への自己投資。積み上げた努力を金額で
  可視化して、続けるモチベーションに変えましょう。」
- ポイント 1: 「カテゴリごとに時給を決める」/「プログラミングや英語など、学びの
  種類ごとに『自分の時間の価値』を設定。」
- ポイント 2: 「タイマーで集中時間を記録」/「計測した時間 × 時給で、その日の
  積み上げ金額を自動計算します。」
- ポイント 3: 「積み上げを『資産』として実感」/「日々の頑張りが金額として
  見えるので、続けるほど自分への投資が楽しくなる。」
...
| 操作 | 成功時 | 失敗時 |
|---|---|---|
| イントロ「はじめる」 | なし（カテゴリステップへ） | 発生しない（純粋な画面遷移のみ） |
