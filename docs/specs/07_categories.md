...
- ステップ 1: 14 個の大カテゴリを 2 列グリッドで表示（`docs/category_master.csv` 順）
...
- マスタデータは `lib/features/category/domain/category_master.dart` に const 定義
  （14 大カテゴリ × 計 65 小カテゴリ）。仕様の正は `docs/category_master.csv`
...
- `lib/features/category/domain/category_master.dart`（プリセット master 65 件）
