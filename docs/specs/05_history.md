...
- アイコン円（カテゴリカラー + アイコン）
- カテゴリ名（`ellipsis`）
  - 旧 入力方式バッジ（タイマー / 手動）は issue #124 で削除。手動入力機能が
    未実装でバッジ表示が情報として機能していなかったため。手動入力機能を
    実装する際にバッジを復活させる前提で `inputMethod` カラムは残置。
...
## リスト表示の上限
- 履歴一覧: 直近 **100 件**（`LimitedSessionGroups.displayLimit`）に絞って表示
  - ただし境界日（100 件目が属する日）のセッションはすべて含める。途中で
    切ると日付ヘッダーの日計が部分合計になるため（issue #202）
  - 表示しきれない件数がある場合は画面側で「N 件以上」の告知に使える
    `totalCount` / `isTruncated` を提供
  - データ量増加時は将来 pagination 検討
- 日付グループ: 終了時刻 (`endTime`) が属する日でグループ化
  - タイマー記録の `endTime` は実際の停止時刻（issue #192）。一時停止・
    課金切り下げ分があるため `startTime + durationSec` とは一致しないことがある

## 状態
- **Loading**: `groupsAsync` 読み込み中は中央 `CircularProgressIndicator`
- **Error**: 「読み込みに失敗しました: $e」
- **Empty**: `_EmptyState`
  - 円形バッジ（96×96、`surfaceContainerHighest` 背景）+ `Icons.history_edu_outlined`
  - 見出し「まだ記録がありません」（titleMedium）
  - 補助文「ホームのタイマーで計測すると / ここに記録が並びます」
